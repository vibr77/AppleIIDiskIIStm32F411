...
static enum STATUS (*disk35GetTrackBitStream)(_2mg_t *,int,unsigned char*);       // pointer to readBitStream function according to driver woz/nic
...
void disk35Init(){
    ...
    if ( mount2mgFile(s2mg,filename)==RET_OK)
        log_info("mounting 2mg OK");
    else
        log_error("mounting 2mg KO");

    disk35GetSDAddr=get2mgSDAddr;
    disk35GetTrackBitStream=get2mgTrackBitStream;
    disk35GetTrackFromPh=get2mgTrackFromPh;
    disk35GetTrackSize=get2mgTrackSize;
                ...
                switch (ctlSwitch) {                                                                // CA1 CA0 SEL CA2 CONT35 Function
                    ...
                    case IWM_DISK35_CTL_STEP_ONE:                                                   //  0   1   0   0   $04     *Reset disk-switched flag? (The firmwareuses this to clear disk-switched errors.)                                                 
                        if (dsk35.status_mask_35 & IWM_DISK35_STATUS_EJECTING) {
                            ...
                            irqEnableSDIO();
                            disk35GetTrackBitStream(s2mg,ph_track,read_track_data_bloc);
                            while(fsState!=READY){}
                            irqDisableSDIO();
